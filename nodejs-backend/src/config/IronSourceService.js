...
class IronSourceService {
  constructor(config) {
    this.apiUrl = config.apiUrl || 'https://api.ironsrc.com/v1';
  ...
  async bid(placementId, userData) {
    try {
      ...
      console.error('IronSource竞价失败:', error);
      return null;
    }
  }
  ...
  render(adData) {
    ...
    container.className = 'ironsource-ads-container';
    ...
    videoContainer.className = 'ironsource-video-container';
    ...
    skipButton.className = 'ironsource-skip-button';
    ...
    progressBar.className = 'ironsource-progress-bar';
    ...
    progress.className = 'ironsource-progress';
  ...
  generateBidId() {
    return `ironsource_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
  }
  ...
  async trackClick(adId) {
    try {
      ...
      console.error('IronSource点击追踪失败:', error);
    }
  }
}

module.exports = IronSourceService; 