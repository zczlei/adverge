...
class AdColonyAdapter extends BaseAdapter {
  constructor(config) {
    super(config, 'AdColony');
    this.apiUrl = config.apiUrl || 'https://api.adcolony.com';
  ...
  async initialize() {
    ...
    try {
      // 加载AdColony SDK
      await this.loadScript('https://sdk.adcolony.com/adcolony-ads-sdk.js');
      
      // 初始化SDK
      window.AdColony.init({
        appId: this.appId,
        zoneId: this.zoneId,
  ...
  async loadAd(adData) {
    try {
      ...
      // 调用AdColony SDK加载广告
      const ad = await window.AdColony.loadAd(adData.placementId);
  ...
  async showAd(container, ad) {
    try {
      ...
      adElement.className = 'adcolony-native-ad';
...
export default AdColonyAdapter; 