...
class MoPubAdapter extends BaseAdapter {
  constructor(config) {
    super(config, 'MoPub');
    this.apiUrl = config.apiUrl || 'https://api.mopub.com';
  ...
  async initialize() {
    ...
    try {
      // 加载MoPub SDK
      await this.loadScript('https://sdk.mopub.com/mopub-ads-sdk.js');
  ...
  async loadAd(adData) {
    try {
      ...
      // 调用MoPub SDK加载广告
      const ad = await window.MoPub.loadAd(adData.placementId);
  ...
  async showAd(container, ad) {
    try {
      ...
      adElement.className = 'mopub-native-ad';
