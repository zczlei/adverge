...
class RubiconAdapter extends BaseAdapter {
  constructor(config) {
    super(config, 'Rubicon');
    this.apiUrl = config.apiUrl || 'https://api.rubicon.com';
  ...
  async initialize() {
    ...
    try {
      // 加载Rubicon SDK
      await this.loadScript('https://sdk.rubicon.com/rubicon-ads-sdk.js');
  ...
  async loadAd(adData) {
    try {
      ...
      // 调用Rubicon SDK加载广告
      const ad = await window.Rubicon.loadAd(adData.placementId);
  ...
  async showAd(container, ad) {
    try {
      ...
      adElement.className = 'rubicon-native-ad';
