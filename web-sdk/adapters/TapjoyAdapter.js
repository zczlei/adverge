...
class TapjoyAdapter extends BaseAdapter {
  constructor(config) {
    super(config, 'Tapjoy');
    this.apiUrl = config.apiUrl || 'https://api.tapjoy.com';
  ...
  async initialize() {
    ...
    try {
      // 加载Tapjoy SDK
      await this.loadScript('https://sdk.tapjoy.com/tapjoy-ads-sdk.js');
  ...
  async loadAd(adData) {
    try {
      ...
      // 调用Tapjoy SDK加载广告
      const ad = await window.Tapjoy.loadAd(adData.placementId);
  ...
  async showAd(container, ad) {
    try {
      ...
      adElement.className = 'tapjoy-native-ad';
