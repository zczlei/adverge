...
class MintegralAdapter extends BaseAdapter {
  constructor(config) {
    super(config, 'Mintegral');
    this.apiUrl = config.apiUrl || 'https://api.mintegral.com';
  ...
  async initialize() {
    ...
    try {
      // 加载Mintegral SDK
      await this.loadScript('https://sdk.mintegral.com/mtg-js-sdk.js');
  ...
  async loadAd(adData) {
    try {
      ...
      // 调用Mintegral SDK加载广告
      const ad = await window.MTG.loadAd(adData.placementId);
  ...
  async showAd(container, ad) {
    try {
      ...
      adElement.className = 'mintegral-native-ad';
...
export default MintegralAdapter; 