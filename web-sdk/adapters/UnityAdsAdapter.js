...
class UnityAdsAdapter extends BaseAdapter {
  constructor(config) {
    ...
    this.apiUrl = config.apiUrl || 'https://api.unity.com';
  ...
  async initialize() {
    ...
    try {
      // 加载UnityAds SDK
      await this.loadScript('https://sdk.unity.com/unity-ads-sdk.js');
  ...
  async loadAd(adData) {
    try {
      ...
      // 调用UnityAds SDK加载广告
      const ad = await window.UnityAds.loadAd(adData.placementId);
  ...
  async showAd(container, ad) {
    try {
      ...
      adElement.className = 'unity-ads-native-ad';
