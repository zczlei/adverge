...
class TikTokAdapter extends BaseAdapter {
  constructor(config) {
    super(config, 'TikTok');
    this.apiUrl = config.apiUrl || 'https://api.tiktok.com';
  ...
  async initialize() {
    ...
    try {
      // 加载TikTok SDK
      await this.loadScript('https://sdk.tiktok.com/tiktok-ads-sdk.js');
  ...
  async loadAd(adData) {
    try {
      ...
      // 调用TikTok SDK加载广告
      const ad = await window.TikTok.loadAd(adData.placementId);
  ...
  async showAd(container, ad) {
    try {
      ...
      adElement.className = 'tiktok-native-ad';
