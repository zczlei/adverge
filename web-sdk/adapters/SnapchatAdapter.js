...
class SnapchatAdapter extends BaseAdapter {
  constructor(config) {
    super(config, 'Snapchat');
    this.apiUrl = config.apiUrl || 'https://api.snapchat.com';
  ...
  async initialize() {
    ...
    try {
      // 加载Snapchat SDK
      await this.loadScript('https://sdk.snapchat.com/snapchat-ads-sdk.js');
      
      // 初始化SDK
      window.Snapchat.init({
        appId: this.appId,
  ...
  async loadAd(adData) {
    try {
      ...
      // 调用Snapchat SDK加载广告
      const ad = await window.Snapchat.loadAd(adData.placementId);
  ...
  async showAd(container, ad) {
    try {
      ...
      adElement.className = 'snapchat-native-ad';
