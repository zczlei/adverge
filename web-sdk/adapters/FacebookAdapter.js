...
class FacebookAdapter extends BaseAdapter {
  constructor(config) {
    ...
    this.apiUrl = config.apiUrl || 'https://api.facebook.com';
  ...
  async initialize() {
    ...
    try {
      // 加载Facebook SDK
      await this.loadScript('https://sdk.facebook.com/facebook-ads-sdk.js');
  ...
  async loadAd(adData) {
    try {
      ...
      // 调用Facebook SDK加载广告
      const ad = await window.FB.loadAd(adData.placementId);
  ...
  async showAd(container, ad) {
    try {
      ...
      adElement.className = 'facebook-native-ad';
