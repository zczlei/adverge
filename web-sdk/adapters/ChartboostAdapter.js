...
class ChartboostAdapter extends BaseAdapter {
  constructor(config) {
    super(config, 'Chartboost');
    this.apiUrl = config.apiUrl || 'https://api.chartboost.com';
  ...
  async initialize() {
    ...
    try {
      // 加载Chartboost SDK
      await this.loadScript('https://sdk.chartboost.com/chartboost-ads-sdk.js');
      
      // 初始化SDK
      window.Chartboost.init({
        appId: this.appId,
  ...
  async loadAd(adData) {
    try {
      ...
      // 调用Chartboost SDK加载广告
      const ad = await window.Chartboost.loadAd(adData.placementId);
  ...
  async showAd(container, ad) {
    try {
      ...
      adElement.className = 'chartboost-native-ad';
...
export default ChartboostAdapter; 