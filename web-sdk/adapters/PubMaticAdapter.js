...
class PubMaticAdapter extends BaseAdapter {
  constructor(config) {
    super(config, 'PubMatic');
    this.apiUrl = config.apiUrl || 'https://api.pubmatic.com';
    this.publisherId = config.publisherId;
    this.adSlotId = config.adSlotId;
  ...
  async initialize() {
    ...
    try {
      // 加载PubMatic SDK
      await this.loadScript('https://sdk.pubmatic.com/pubmatic-ads-sdk.js');
      
      // 初始化SDK
      window.PubMatic.init({
  ...
  async loadAd(adData) {
    try {
      ...
      // 调用PubMatic SDK加载广告
      const ad = await window.PubMatic.loadAd(adData.placementId);
  ...
  async showAd(container, ad) {
    try {
      ...
      adElement.className = 'pubmatic-native-ad';
