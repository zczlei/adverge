...
class AppLovinAdapter extends BaseAdapter {
  constructor(config) {
    super(config, 'AppLovin');
    this.apiUrl = config.apiUrl || 'https://api.applovin.com';
  ...
  async initialize() {
    ...
    try {
      // 加载AppLovin SDK
      await this.loadScript('https://sdk.applovin.com/applovin-ads-sdk.js');
  ...
  async loadAd(adData) {
    try {
      ...
      // 调用AppLovin SDK加载广告
      const ad = await window.AppLovin.loadAd(adData.placementId);
  ...
  async showAd(container, ad) {
    try {
      ...
      adElement.className = 'applovin-native-ad';
