...
class AdMobAdapter extends BaseAdapter {
  constructor(config) {
    super(config, 'AdMob');
    this.apiUrl = config.apiUrl || 'https://api.admob.com';
  ...
  async initialize() {
    ...
    try {
      // 加载AdMob SDK
      await this.loadScript('https://sdk.admob.com/admob-ads-sdk.js');
  ...
  async loadAd(adData) {
    try {
      ...
      // 调用AdMob SDK加载广告
      const ad = await window.AdMob.loadAd(adData.placementId);
