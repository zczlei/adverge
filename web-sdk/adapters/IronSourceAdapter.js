...
class IronSourceAdapter extends BaseAdapter {
  constructor(config) {
    super(config, 'IronSource');
    this.apiUrl = config.apiUrl || 'https://api.ironsrc.com';
  ...
  async initialize() {
    ...
    try {
      // 加载IronSource SDK
      await this.loadScript('https://sdk.ironsrc.com/iron-ads-sdk.js');
  ...
  async loadAd(adData) {
    try {
      ...
      // 调用IronSource SDK加载广告
      const ad = await window.IronSource.loadAd(adData.placementId);
  ...
  async showAd(container, ad) {
    try {
      ...
      adElement.className = 'ironsource-native-ad';
