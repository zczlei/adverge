...
class PangleAdapter extends BaseAdapter {
  constructor(config) {
    super(config, 'Pangle');
    this.apiUrl = config.apiUrl || 'https://api.pangle.com';
  ...
  async initialize() {
    ...
    try {
      // 加载Pangle SDK
      await this.loadScript('https://sdk.pangle.com/pangle-ads-sdk.js');
  ...
  async loadAd(adData) {
    try {
      ...
      // 调用Pangle SDK加载广告
      const ad = await window.Pangle.loadAd(adData.placementId);
  ...
  async showAd(container, ad) {
    try {
      ...
      adElement.className = 'pangle-native-ad';
